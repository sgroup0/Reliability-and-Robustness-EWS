...
p_list_rms_bifn = zeros(n, length(mu_list));
p_list_var_bifn = zeros(n, length(mu_list));
p_list_sk_bifn = zeros(n, length(mu_list));
p_list_kr_bifn = zeros(n, length(mu_list));
p_list_AC_bifn = zeros(n, length(mu_list));
p_list_H_bifn = zeros(n, length(mu_list));

% Allocate space for timeseries till max rate of change of rms
p_list_rms_maxrate = zeros(n, length(mu_list));
p_list_var_maxrate = zeros(n, length(mu_list));
p_list_sk_maxrate = zeros(n, length(mu_list));
p_list_kr_maxrate = zeros(n, length(mu_list));
p_list_AC_maxrate = zeros(n, length(mu_list));
p_list_H_maxrate = zeros(n, length(mu_list));

% Allocate space for timeseries till maxima
p_list_var_maxima = zeros(n, length(mu_list));
p_list_sk_maxima = zeros(n, length(mu_list));
p_list_kr_maxima = zeros(n, length(mu_list));
p_list_AC_maxima = zeros(n, length(mu_list));
p_list_H_maxima = zeros(n, length(mu_list));
% Allocate space for H of MMK of timeseries till maxima
H_list_var_maxima = zeros(n, length(mu_list));
H_list_sk_maxima = zeros(n, length(mu_list));
H_list_kr_maxima = zeros(n, length(mu_list));
H_list_AC_maxima = zeros(n, length(mu_list));
H_list_H_maxima = zeros(n, length(mu_list));

% Averaged
% Allocate space for timeseries till maxima
p_list_var_maxima_avg = zeros(1, length(mu_list));
p_list_sk_maxima_avg = zeros(1, length(mu_list));
p_list_kr_maxima_avg = zeros(1, length(mu_list));
p_list_AC_maxima_avg = zeros(1, length(mu_list));
p_list_H_maxima_avg = zeros(1, length(mu_list));
% Allocate space for H of MMK of timeseries till maxima
H_list_var_maxima_avg = zeros(1, length(mu_list));
H_list_sk_maxima_avg = zeros(1, length(mu_list));
H_list_kr_maxima_avg = zeros(1, length(mu_list));
H_list_AC_maxima_avg = zeros(1, length(mu_list));
H_list_H_maxima_avg = zeros(1, length(mu_list));
...
for k = 1: n
    ...
    for i = 1: length(mu_list)
        ...
        % Call the function
        [transition_time_calc, Ktau_bifn, Ktau_maxrate, Ktau_maxima] = EWS_Ktau_Estimate(k, i, window_size_in_seconds, time_transient, overlap_ratio);
    
        % Assign p-value for timeseries till bifurcation
        p_list_rms_bifn(k, i) = Ktau_bifn.p.rms;
        p_list_var_bifn(k, i) = Ktau_bifn.p.var;
        p_list_sk_bifn(k, i) = Ktau_bifn.p.sk;
        p_list_kr_bifn(k, i) = Ktau_bifn.p.kr;
        p_list_AC_bifn(k, i) = Ktau_bifn.p.AC;
        p_list_H_bifn(k, i) = Ktau_bifn.p.H;
    
        % Assign p-value for timeseries till max rate of change of rms
        p_list_rms_maxrate(k, i) = Ktau_maxrate.p.rms;
        p_list_var_maxrate(k, i) = Ktau_maxrate.p.var;
        p_list_sk_maxrate(k, i) = Ktau_maxrate.p.sk;
        p_list_kr_maxrate(k, i) = Ktau_maxrate.p.kr;
        p_list_AC_maxrate(k, i) = Ktau_maxrate.p.AC;
        p_list_H_maxrate(k, i) = Ktau_maxrate.p.H;
    
        % Assign p-value for timeseries till maxima of corresponding signal
        p_list_var_maxima(k, i) = Ktau_maxima.p.var;
        p_list_sk_maxima(k, i) = Ktau_maxima.p.sk;
        p_list_kr_maxima(k, i) = Ktau_maxima.p.kr;
        p_list_AC_maxima(k, i) = Ktau_maxima.p.AC;
        p_list_H_maxima(k, i) = Ktau_maxima.p.H;
        % Assign H-value for each of the EWS
        H_list_var_maxima(k, i) = Ktau_maxima.H.var;
        H_list_sk_maxima(k, i) = Ktau_maxima.H.sk;
        H_list_kr_maxima(k, i) = Ktau_maxima.H.kr;
        H_list_AC_maxima(k, i) = Ktau_maxima.H.AC;
        H_list_H_maxima(k, i) = Ktau_maxima.H.H;
...
for i = 1: length(mu_list)
    ...
    % Var
    H_list_var_maxima_avg(i) = round(sum(H_list_var_maxima(1:n, i)) / n);
    p_list_var_maxima_avg(i) = sum(p_list_var_maxima(1:n, i)) / n;

    % Sk
    H_list_sk_maxima_avg(i) = round(sum(H_list_sk_maxima(1:n, i)) / n);
    p_list_sk_maxima_avg(i) = sum(p_list_sk_maxima(1:n, i)) / n;

    % Kr
    H_list_kr_maxima_avg(i) = round(sum(H_list_kr_maxima(1:n, i)) / n);
    p_list_kr_maxima_avg(i) = sum(p_list_kr_maxima(1:n, i)) / n;

    % AC
    H_list_AC_maxima_avg(i) = round(sum(H_list_AC_maxima(1:n, i)) / n);
    p_list_AC_maxima_avg(i) = sum(p_list_AC_maxima(1:n, i)) / n;

    % H
    H_list_H_maxima_avg(i) = round(sum(H_list_H_maxima(1:n, i)) / n);
    p_list_H_maxima_avg(i) = sum(p_list_H_maxima(1:n, i)) / n;

end

% Assign values to structures to give them as inputs to functions
H_list_maxima.var = H_list_var_maxima_avg;
H_list_maxima.sk = H_list_sk_maxima_avg;
H_list_maxima.kr = H_list_kr_maxima_avg;
H_list_maxima.AC = H_list_AC_maxima_avg;
H_list_maxima.H = H_list_H_maxima_avg;

p_list_maxima.var = p_list_var_maxima_avg;
p_list_maxima.sk = p_list_sk_maxima_avg;
p_list_maxima.kr = p_list_kr_maxima_avg;
p_list_maxima.AC = p_list_AC_maxima_avg;
p_list_maxima.H = p_list_H_maxima_avg;
...
function [time_max_rate, Ktau_bifn, Ktau_maxrate, Ktau_maxima] = EWS_Ktau_Estimate(data_idx, rate_idx, window_size_in_seconds, time_transient, overlap_ratio)
%% INITIAL SETUP
...
% Load timeseries
filename = sprintf('../Data/Noise5_Many/%d/NoiseOmega5_delta%.2f_omega%.2f_E%.2f_Pm%.4f_mu%.5f_t%.2f_deltaT%.5f_ConstantTimeStep.mat', data_idx, delta0, omega0, E0, Pm0, mu, t2, delta_t);
Data = load(filename);
