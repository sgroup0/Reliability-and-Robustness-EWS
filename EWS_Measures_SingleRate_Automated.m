...
%% RUN THE FUNCTION FOR DIFFERENT RATES AND ANALYZE THE FIGURES PRODUCED
...
% Run the analysis on time series generated at each rate, also fill in the table
for i = 1: length(mu_list)
    ...
    % Call the function
    transition_time_calc = EWS_Measures_SingleRate_Automated_func(i, window_size_in_seconds, time_transient, overlap_ratio);
...
%% FUNCTION WHICH PLOTS THE FIGURES FOR THE TIMESERIES GENERATED AT DIFFERENT RATES OF CHANGE OF PARAMETER

function time_max_rate = EWS_Measures_SingleRate_Automated_func(rate_idx, window_size_in_seconds, time_transient, overlap_ratio)
%% INITIAL SETUP
